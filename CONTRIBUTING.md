...
Thanks for improving Layout Presets.

End users install with `brew tap BadryansahBangsawan/mac-menu-apps` and `brew install --cask layout-presets`. This file is for source contributors.
...
Do not commit `dist/`, `.build/`, `.swiftpm/`, or secrets. Do not run `dist/` next to `/Applications/LayoutPresets.app` (same bundle ID).
...
- Accessibility is bound to the ad-hoc **cdhash**. After rebuild, Settings can show an old enabled row while the running process is untrusted. Offer toggle off/on plus **Relaunch**. Do not call `isTrusted(prompt: true)` on every capture/restore.
